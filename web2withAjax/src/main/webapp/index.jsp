                ...
                <table class="_form">
                    ...
                    <tr>
                        <td class="submit-row" align="center">
                            <br>
                            <button id="okBut" type="submit">OK</button>
                        </td>
                    </tr>
                </table>
            </form>
            <form method="get" action="${pageContext.request.contextPath}/ControllerServlet">
                <table>
                    <tr>
                        <td>
                            <br>
                            <input type="hidden" name="x_value" value="1">
                            <input type="hidden" name="y_value" value="1">
                            <input type="hidden" name="radius" value="1">
                            <input type="hidden" name="check" value="NO">
                            <button id="lookBut" type="submit">Посмотреть таблицу</button>
                        </td>
                    </tr>
                </table>
            </form>
                ...
                <circle id="circle"
                        <%
                            if(application.getAttribute("requestsCount") == null){
                                out.print(" r='5' cx='150' cy='150' fill='white' stroke='black' ");
                            } else {
                                double x, y, radius;
                                ...
                                out.print(String.format(" r='5' cx='%s' cy='%s' fill='%s' stroke='black' ",
...
<script type="text/javascript">
    ...
    function getFillColor(x, y, r){
        if (((x > 0) && (y > 0) && (x + y <= r))
            || ((x < 0) && (y > 0) && (y <= r) && (x >= -r))
            || ((x < 0) && (y < 0) && ((x * x) + (y * y) <= r * r))
            || ((x == 0) && (((y >= 0) && (y <= r)) || ((y <= 0) && (y >= -r))))
            || ((y == 0) && (((x >= 0) && (x <= r)) || ((x <= 0) && (x >= -r))))) return "white";
        else return "black";
    }

    /*var okButton = document.getElementById("okBut");
    ...
    svg.addEventListener("click", function (ev) {
        ...
        var fillColor = getFillColor(x, y, radius);
